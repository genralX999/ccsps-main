...
$users = $pdo->query('SELECT id, monitor_id_code, username, email, role, status FROM users ORDER BY id DESC')->fetchAll();
ob_start();
?>
...
<div class="bg-white p-4 rounded shadow">
  ...
  <table class="w-full table-auto">
    <thead><tr><th>ID</th><th>Code</th><th>Username</th><th>Email</th><th>Role</th><th>Status</th><th>Actions</th></tr></thead>
    <tbody>
      <?php foreach($users as $u): ?>
      <tr class="border-t" data-user='<?= json_encode($u, JSON_HEX_APOS|JSON_HEX_QUOT) ?>'>
        <td><?= $u['id'] ?></td>
        <td><?= htmlspecialchars($u['monitor_id_code']) ?></td>
  ...
  <div class="bg-white rounded shadow w-full max-w-md p-4">
    ...
    <form id="editForm">
      ...
      <label class="block mb-2">Role
        <select name="role" class="w-full p-2 border rounded"><option value="user">User</option><option value="admin">Admin</option><option value="superadmin">Superadmin</option></select>
      </label>
